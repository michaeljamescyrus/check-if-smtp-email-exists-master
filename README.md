...
| 🔜        | **Free email provider check**                 | Is the email address bound to a known free email provider?                                                                      | [Issue #89](https://github.com/michaeljamescyrus/check-if-email-exists/issues/89)   |
| 🔜        | **Syntax validation, provider-specific**      | According to the syntactic rules of the target mail provider, is the address syntactically valid?                               | [Issue #90](https://github.com/michaeljamescyrus/check-if-email-exists/issues/90)   |
| 🔜        | **Honeypot detection**                        | Does email address under test hide a [honeypot](https://en.wikipedia.org/wiki/Spamtrap)?                                        | [Issue #91](https://github.com/michaeljamescyrus/check-if-email-exists/issues/91)   |
| 🔜        | **Gravatar**                                  | Does this email address have a [Gravatar](https://gravatar.com/) profile picture?                                               | [Issue #92](https://github.com/michaeljamescyrus/check-if-email-exists/issues/92)   |
| 🔜        | **Have I Been Pwned?**                        | Has this email been compromised in a [data breach](https://haveibeenpwned.com/)?                                                | [Issue #289](https://github.com/michaeljamescyrus/check-if-email-exists/issues/289) |
...
There are a few ways you can try `check-if-email-exists`.

### 2. Deploy to Heroku

[![Deploy](https://www.herokucdn.com/deploy/button.svg)](https://heroku.com/deploy?template=https://github.com/michaeljamescyrus/check-if-email-exists)
...
The [Docker image](./Dockerfile) is hosted on Docker Hub: https://hub.docker.com/r/michaeljamescyrus/check-if-email-exists.

To run it, run the following command:

```bash
docker run -p 3000:3000 michaeljamescyrus/check-if-email-exists
```
...
Head to the [releases page](https://github.com/michaeljamescyrus/check-if-email-exists/releases) and download the binary for your platform. Make sure you have [`openssl`](https://www.openssl.org/) installed on your local machine.
...
```bash
# Download the code
$ git clone https://github.com/michaeljamescyrus/check-if-email-exists
...
The 1st version of this tool was a simple bash script which made a telnet call. If you would like to use that simpler version, have a look at the [`legacy`](https://github.com/michaeljamescyrus/check-if-email-exists/tree/legacy) branch. The reasons for porting the bash script to the current codebase are explained [in issue #4](https://github.com/michaeljamescyrus/check-if-email-exists/issues/4).
